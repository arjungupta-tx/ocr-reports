                                ...
                                if file_path1 is not None:
                                    ...
                                    if st.session_state.page_count >0:
                                        ...
                                        for k,i in enumerate(st.session_state.list_images):
                                            ...
                                            # txt_docai = f"""{txt_docai}""".replace("\n", "")
                                            # txt_aponai = f"""{txt_aponai}""".replace("\n", "")
                                            # txt_anthropic = f"""{txt_anthropic}""".replace("\n", "")
                                            # txt_comperision = f"""{txt_comperision}""".replace("\n", "")


                                            # query_insert = f"""INSERT INTO Ocr_Page(Ocr_Accuracy,Ocr_Document_AI,Ocr_Open_AI,Ocr_Anthropic,Ocr_Comparable,Total_Time_Document_AI,Total_Time_Open_AI,Total_Time_Anthropic,
                                            #                                         Total_Token_Document_AI,Total_Token_Open_AI,Total_Token_Athropic,Total_Price_Document_AI,Total_Price_Open_AI,Total_Price_Anthropic,Id_Pdf,Pgae_Number,
                                            #                                         Total_Token_Comperision_api,Total_Time_Comperision,Txt_Update_Status,Date)
                                            #                                         VALUES({int(accuracy_docai)},'{str(txt_docai)}','{str(txt_aponai)}','{str(txt_anthropic)}','{str(txt_comperision)}',
                                            #                                         '{str(time_docai)}','{str(time_openai)}','{str(time_anthropic)}',{int(0)},{int(toketopenai)},{int(token_anthropic)},
                                            #                                         {float(0.00)},{float(0.00)},{float(0.00)},{int(pdf_id)},{int(k+1)},{int(openai_token_compare)},'{str(time_coperision)}','Row Text','{str(datetime.now().strftime('%Y-%m-%d'))}');"""
