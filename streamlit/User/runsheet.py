                                            ...
                                            if count[0] == st.session_state.page_count:
                                                ...
                                                if result_retrieve:
                                                    with st.container(border=True):
                                                        ...
                                                        runshetbtn = st.download_button(
                                                                            label="Download OCR And Runsheet",
                                                                            data=content,   
                                                                            file_name="runsheet.txt",
                                                                            mime="text/plain"

                                                                        )
                                                       
                                                        if runshetbtn:
                                                            st.write("Runsheet Downloaded")
                                            else:
                                ...
                                if file_path1 is not None:
                                    ...
                                    if st.session_state.page_count >0:
                                        ...
                                        for k,i in enumerate(st.session_state.list_images):
                                            ...
                                            # progress_bar.progress(int(((k + 1)/(st.session_state.page_count))*100),text=status_text)
                                            # promt_txt=Prompt_txt_ocr
                                            st.session_state.df.at[k, "Status"] = "In Progress"
                                            ...
                                            if result.get("rows_affected",0) > 0:
                                                with st.container(border=True):
                                                    st.session_state.df.at[k, "Details"] = f"OCR Progress: {k+1}/{st.session_state.page_count}"
                                                                    # st.session_state.df_style=st.session_state.df.style.applymap(color_status, subset=['Status'])
                                                    st.session_state.table_placeholder.dataframe(st.session_state.df,use_container_width=True) 
